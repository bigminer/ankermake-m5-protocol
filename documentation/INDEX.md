...
| F-036 | **The working flow injects `M4899 T3` right after `G28`** (S-curve motion + new linear-advance model, `LIN_ADV_VERSION_3`), plus `M900 T0 K0.0x` and `M205`, via the slicer profile's `gcode_substitutions`. **We send none of them** — `ankerctl` uploads byte-for-byte (`web/util.py:223`), so this bites only non-AnkerStudio slicers. ⚠️ **That it changes a printed part is `UNVERIFIED`** — do not act as if it does. Tracked by **issue #32** (investigate, then decide); the suspected defect is non-determinism, since `M4899` mutates planner settings in RAM with nothing resetting them per job | `planner.h:189`; `AnkerMake base/base.ini:73` |
