...
| plan to read settings with `M503` (or any long reply) over `1043` | **It truncates.** `M503` returns a lossy tail; bare `M204` dies mid-word at 32 chars. Only replies that *lead* with payload survive | F-040 |
...
| F-040 | **Long G-code replies do not survive the `1043` transport.** `M503` returns only a lossy tail; bare `M204` truncates mid-word at 32 chars (`…+ringbuf:1,512,0\nAcceler`), byte-identical across three sends. `M114` survives because it *leads* with payload — `M204` leads with ~25 bytes of acks. **Not our bug**: `resData` is absent from `libflagship/`/`specification/`. **Reading stored motion limits needs serial or an app capture, not another `M503`.** Obs `CONFIRMED` 2026-07-31 | findings §"The `M503` read, run 2026-07-31" |
| F-041 | **No `M4899` has run since this printer's last boot**, so it is in `LIN_ADV_VERSION_0`. Every branch sets the hybrid threshold explicitly — T0/T1 → `X329 Y329`, T2/T3 → `X0 Y0`; the machine reports **`X333 Y333`**, neither. ⚠️ 333 is also not the compile-time default (329, `Configuration_adv.h:2924`), so **its EEPROM has been written with customised settings** — the F-039 hybrid risk is live and unresolved. Obs `STRONG` | `M4200_M4900.cpp:248/259/289/320` |
