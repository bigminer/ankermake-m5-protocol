...
| plan to read settings with `M503` (or any long reply) over `1043` | **Capped at 512 bytes, and spliced above it** — a joined line can look valid but never existed. Use a command that fits under the cap | F-040 |
...
| F-040 | **`1043` reply payloads are capped at 512 bytes — and over the cap they are *spliced*, not just truncated.** Measured: `M114` 64 B, `M913` 164 B (both complete); `M115` and `M503` exactly 512 B. The `M503` frame joins two points in the stream, and **the splice position drifts between runs**. 🚨 **A spliced frame can present a syntactically valid line that never existed** — re-read any such value with a command that fits under the cap. **Not our bug** (`resData` absent from `libflagship/`; `json.loads` would raise). Tier 2 corroboration: [Ankermgmt #48](https://github.com/Ankermgmt/ankermake-m5-protocol/issues/48) reports "a section of the middle" for `M503`. Obs `CONFIRMED` 2026-07-31 | findings §"The `M503` read, run 2026-07-31" |
